...
**REVIEW GATE #4 — Codex review of idempotency scope.** Focus: every `deleteMany` / `findFirst` / `upsert` call site in the persist path includes `contractId`; the IMPORTANT 3 test catches the regression mode; no implicit Plan 02 I.1 FIX 1 lingering scope. Findings → micro-commits before Phase F starts.
