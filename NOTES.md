...
- **Module implementation registry pathway (Plan 03+).** Plan 02 H.6 introduces `IMPLEMENTED_MODULES` (single source of truth, `src/lib/scan-submission.ts`) as the gate that decides whether a submitted module ships QUEUED or SKIPPED. Today the set contains only `GOVERNANCE`; ORACLE / SIGNER / FRONTEND ship SKIPPED with `errorMessage = "module_not_implemented"`. **When implementing a new module:** (1) add the Inngest function under `src/lib/inngest/functions/execute-<module>-module.ts`, (2) register it in `src/app/api/inngest/route.ts`, (3) add the `ModuleName` enum value to `IMPLEMENTED_MODULES`, (4) update `scan-submission-integration.test.ts` assertions for the now-implemented module, (5) remove the module from `PLACEHOLDER_MODULES` in `src/lib/__tests__/scan-submission-modules.test.ts` so the completeness guard re-arms. UX polish: `ModuleCard` SKIPPED copy currently reads "Not included in this scan" — Phase G visual-polish backlog could differentiate "not included" vs "coming soon" once we have more than one placeholder reason.
